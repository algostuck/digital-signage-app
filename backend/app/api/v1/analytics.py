...
@router.get("/analytics/aggregates", dependencies=[require_permissions("reports.view")])
async def aggregates(
    ...
    return success(
        await service.list_aggregates(
            db, tenant_id, dimension_type=dimension_type,
            date_from=date_from, date_to=date_to,
        )
    )


@router.get("/analytics/reconciliation", dependencies=[require_permissions("reports.view")])
...
@router.get("/data-exports", dependencies=[require_permissions("reports.export")])
...
@router.post(
    "/data-exports", dependencies=[require_permissions("reports.export")], status_code=201
)
async def create_export(
    ...
    export = await service.create_export(
        db, tenant_id, name=body.name, dataset=body.dataset,
        cadence=body.cadence, user_id=user.id,
    )
