...
| P3-09 | Ad Inventory | /ads → Inventory tab | advertising flag + ads.manage | /ad-inventory | slot table, hours editor, device scope | **done (3D-1)** |
| P3-10 | Ad Campaign Manager | /ads → Bookings tab | ads.manage | /ad-campaigns | booking form (slot/campaign/window/units), approval hand-off, delivered counts, cancel | **done (3D-1)** |
| P3-11 | Ad Performance Report | /reports → Ads tab | reports.view | /reports/ad-performance (+export) | booked-vs-delivered-vs-fill-rate table, 2I export buttons | **done (3D-2)** |
